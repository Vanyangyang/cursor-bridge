...
- `on`: explicitly activate Grok Executor Mode for the current host task. Do not open a TUI or send work merely to activate it. Confirm briefly. From the next ordinary user task onward, automatically apply `$grok-executor-mode` until an exact `/grok_execute off` or the host task ends. Repeated `on` is idempotent.
