...
- For `/grok_execute on`, return one concise standalone result only after the visible TUI is ready: `Grok 执行模式已开启，终端已就绪，直接告诉我任务即可。` For ordinary task dispatch, report the task state or outcome instead of narrating TUI lifecycle details.
...
5. After `grok_session_prompt` returns, keep the current host turn alive until it reaches an attention or terminal state:
   ...
   - While state is `working` or `cancelling`, use only the compact state and progress metadata. Streamed agent message bodies are suppressed until a terminal result, so do not try to reconstruct the answer from polling updates. A bounded wait timeout is not completion.
