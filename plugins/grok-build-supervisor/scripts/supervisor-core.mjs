...
const MAX_UNKNOWN_ACK_REASON_CHARS = 1000;
...
export function deriveRecoveryState(events) {
  const terminalRuns = new Set(events
    .filter((event) => new Set(["prompt_completed", "prompt_failed"]).has(event.kind))
    .map((event) => event.runId)
    .filter(Boolean));
  const acknowledgedUnknownRuns = new Set(events
    .filter((event) => event.kind === "unknown_run_acknowledged")
    .map((event) => `${event.sessionId}:${event.runId}`));
  const interruptedRun = [...events].reverse()
    .find((event) => event.kind === "prompt_started"
      && event.runId
      && !terminalRuns.has(event.runId)
      && !acknowledgedUnknownRuns.has(`${event.sessionId}:${event.runId}`));
...
export class GrokSupervisor {
  ...
  readRawActiveSessions() {
    ...
    try {
      const sessions = collectActiveSessions(JSON.parse(readFileSync(registry, "utf8")));
      this.activeSessionsReadError = null;
      return sessions;
    } catch (error) {
      this.activeSessionsReadError = conciseError(error);
  ...
  latestRunSnapshot({ sessionId = null, runId = null } = {}) {
    ...
    return {
      ...
      completedAt: new Set(["prompt_started", "unknown_run_acknowledged"]).has(latest.kind) ? null : latest.timestamp,
      terminalSequence: new Set(["prompt_started", "unknown_run_acknowledged"]).has(latest.kind) ? null : latest.sequence,
  ...
  unknownAcknowledgementBlockers(sessionId) {
    ...
    if ((this.acpConnection && !this.acpConnection.signal.aborted)
      ...
      || (this.acpProcess?.pid && this.processIsAlive(this.acpProcess.pid))) {
    ...
    const ownership = this.readLeaderOwnership();
    const leaderLockExists = existsSync(this.leaderLockPath());
    ...
    if (Number.isInteger(recordedLeaderPid)) {
      ...
      const recordedLeaderAlive = typeof recordedFingerprint === "string" && recordedFingerprint.length > 0
        ? ["matching", "unknown_alive"].includes(this.recordedProcessIdentityState(
          recordedLeaderPid,
          recordedFingerprint,
          ownership.record?.executablePath || null,
        ).state)
        : this.processIsAlive(recordedLeaderPid);
      if (recordedLeaderAlive) {
        blockers.push("live_or_uncertain_owned_leader");
      }
    }
    if (Number.isInteger(leaderLockPid)
      && leaderLockPid !== recordedLeaderPid
      && this.processIsAlive(leaderLockPid)) {
      blockers.push("live_or_uncertain_owned_leader");
    }
    ...
    if ([...this.tuiProcesses.entries()].some(([pid, owned]) =>
      owned?.sessionId === sessionId && this.ownedTuiIdentityMatches(pid, owned))) {
