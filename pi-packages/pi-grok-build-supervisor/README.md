...
Select `grok-executor-on` or `grok-executor-off` to enable or disable execution without supplying an extra argument. The package includes four Supervisor Skills, both compatibility prompt templates, the bundled MCP frontend and daemon, and the visible Windows Terminal TUI runtime. It embeds Grok Build Supervisor 0.4.1 and retains the same trust, permission, ownership, compact-progress, and result-artifact boundaries documented by the main plugin. Pi is carried through the authenticated host-identity envelope and is never mislabeled as Codex.
