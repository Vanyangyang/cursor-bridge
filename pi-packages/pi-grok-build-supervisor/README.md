...
The package includes the two Supervisor Skills, both prompt templates, the bundled MCP frontend and daemon, and the visible Windows Terminal TUI runtime. It embeds Grok Build Supervisor 0.3.7 and retains the same trust, permission, ownership, compact-progress, and result-artifact boundaries documented by the main plugin. Pi is carried through the authenticated host-identity envelope and is never mislabeled as Codex. User-facing explanations follow the current task language while machine fields and exact permission/form options remain unchanged.

Full documentation: [English](https://github.com/Vanyangyang/cursor-bridge/tree/master/plugins/grok-build-supervisor#readme) · [简体中文](https://github.com/Vanyangyang/cursor-bridge/blob/master/plugins/grok-build-supervisor/README.zh-CN.md)
