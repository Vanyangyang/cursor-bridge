...
function createWorkspaceBindingError(diagnostic) {
  const error = new Error(`Cursor Agents workspace binding failed: ${diagnostic.state || 'unknown'}; wanted=${diagnostic.wanted || diagnostic.workspace || 'unknown'}. ${diagnostic.nextStep || 'Re-run cursor_init for the exact local workspace.'}`);
  error.code = 'CURSOR_WORKSPACE_BINDING_FAILED';
  error.workspaceBindingFailure = diagnostic;
...
function normalizeModelPickerModelText(value) {
  ...
  return normalized.startsWith('cursor') && normalized.length > 'cursor'.length
...
// Agents Window groups histories by repository identity, not local directory.
// Only a registered workspace's file URI identifies the requested checkout.
// Historical Agent environments and remote aliases are diagnostics, never a
// substitute for a registered creation target (observed in Cursor 3.19.13).
const WORKSPACE_SECTION_BODY = String.raw`
...
// Explicit cursor_init only: use the same project service as Open Workspace.
// Never use repository labels or a fabricated workspace identifier to register.
...
class CursorBridge {
  constructor(options = {}) {
    ...
    this.workspaceConfirmationRequired = this.workspaceKey === 'default' && options.projectPath === undefined;
  ...
  workspaceView() {
    ...
    return {
      ...
      workspaceBindingWarning: this.workspaceConfirmationRequired
        ? 'This adapter has no host workspace identity. Run cursor_init with the intended project before using its shared saved binding.' : null,
  ...
  _claimExistingSession({ sessionId, taskId, projectPath, readOnly, allowedPaths, requestId, timeoutMs }) {
    ...
    const result = updateCursorSessionRegistry(this.sessionFile, (registry) => {
      ...
      if (session.recoveryState === 'reconciled_result_uncollected'
        && (Number(session.turnIndex) <= 1 || session.responseBaseline)) {
  ...
  async _reconcileSession(sessionId, { collectResult = false } = {}) {
    ...
    const unreadAfterRestart = initial.lastTask?.status === 'completed' && !initial.lastTask.resultCollectedAt
      && !this.tasks.has(initial.lastTask.taskId);
    ...
    if (collectResult && (initial.state !== 'ready' || initial.activeTaskId || initial.lastTask?.status !== 'completed'
      || initial.recoveryState !== 'reconciled_result_uncollected')) {
    ...
    let collectedReply;
    if (collectResult) {
      if (!observed.stable || !observed.entry || observed.entry.showSpinner
        || classifyParallelTerminalIcon(observed.entry.icon) !== 'completed') {
      ...
      try {
        ...
        if (probe.uiDiagnostic && error && typeof error === 'object') error.uiDiagnostic = probe.uiDiagnostic;
        throw error;
      }
    ...
    updateCursorSessionRegistry(this.sessionFile, (registry) => {
      ...
      if (Number(session.epoch || 0) !== Number(initial.epoch || 0)
        || session.activeTaskId !== initial.activeTaskId || session.state !== initial.state) {
      ...
      session.attention = terminalClass === 'completed'
        ? (Number(session.turnIndex) > 1 && !session.responseBaseline
          ? 'This older interrupted turn has no saved reply signature. Inspect the original Agent manually before continuing; automatic collection cannot identify its reply safely.'
          : 'The interrupted turn completed in Cursor, but its reply was not persisted. Use cursor_session_control(action=collect_result) before the next turn.')
        : null;
  ...
  async _initializeWorkspace(projectPath) {
    // Finish a prior read-only heal before changing its target or requesting registration.
    ...
    try {
      ...
      if (registerWorkspace && repository.state === 'workspace_registration_required') {
        const registration = await this._withUiLock(async () => {
          ...
          return JSON.parse(await evalJS(c, exprRegisterAgentsWorkspace(projectPath, statSync(projectPath).isFile())) || '{}');
        });
      ...
      this._lastWorkspaceBinding = { ...repository, checkedAt: new Date().toISOString() };
      ...
      this._lastWorkspaceBinding = { ok: false, state: 'workspace_probe_failed', wanted: projectPath, error: String(error.message || error) };
      return null;
    } finally {
      c.close();
    }
  ...
  async _prepareTask(prompt, options = {}) {
    ...
    const requestedTimeoutMs = Number(options.timeoutMs ?? 600000);
    if (!Number.isFinite(requestedTimeoutMs)) throw new Error('timeout_ms must be finite');
    const timeoutMs = Math.max(30000, Math.min(900000, requestedTimeoutMs));
    ...
    if (sessionMode === 'create') {
      ...
      if (duplicate) {
        ...
        const response = existing
          ? { duplicate: true, ...(options.background === false
            ? this._collectedTaskView(existing)
            : this._compactTaskView(existing)) }
  ...
  _withWorkspaceAdmission(fn) {
    ...
    const run = this._workspaceTail.then(fn, fn).finally(() => { this._workspacePending--; });
  ...
  _ensureCursor({ registerWorkspace = false } = {}) {
    ...
    this._healing = (async () => {
      try {
        ...
        // An already-open IDE folder does not register an Agents creation target.
        // Check the preferred Agents Window even when lifecycle found an IDE match.
  ...
  async _drain() {
    ...
    try {
      ...
      this._orphanParallelJob(job, new Error('Cancellation was requested, but Cursor could not be confirmed stopped; the reservation remains held'));
            ...
            job.firstWaitError ||= error.message;
      ...
      try {
        ...
        if (options.newChat !== false) {
          try {
            ...
            if (attempt === 0 && options.targetUiFlavor === 'agents_v2' && isAgentsWorkspaceBindError(error) && !error.workspaceBindingFailure) {
  ...
  async _newChat(c, options = {}) {
    if (options.uiFlavor === 'agents_v2' && options.projectPath) {
      ...
      if (options.job) {
        options.job.workspaceBindingChecks = { before_create: { ...created, checkedAt: new Date().toISOString() } };
      }
      if (!created.ok) {
  ...
  async _verifyAgentsWorkspace(c, job, stage = 'before_fill') {
    ...
    job.workspaceBinding = { ...diagnostic, checkedAt: new Date().toISOString() };
  ...
  async _confirmSubmission(c, baselineCount = 0, providerErrorBaseline = '', job = null) {
    ...
    try {
      ...
      // Enter was already dispatched. A changed target prevents another click,
      // but cannot prove that the first submission was never accepted.
      error.confirmedNotSent = false;
      error.sent = true;
      throw error;
    }
    const clicked = await evalJS(c, EXPR_CLICK_SEND);
    // Enter can be accepted between the last snapshot and the fallback click.
    // A missing/disabled Send button is not evidence that nothing was submitted.
    ...
    const error = new Error(`Cursor submission could not be confirmed (submission_uncertain: ${clicked || 'unknown'}); Enter was dispatched, so inspect the bound Agent before retrying`);
    error.confirmedNotSent = false;
    error.sent = true;
    error.composerDiagnostic = {
    ...
    try {
      ...
      try {
        createdForWorkspace = job.targetUiFlavor === 'agents_v2' && job.projectPath
          ? await this._newChat(c, { uiFlavor: job.targetUiFlavor, projectPath: job.projectPath, job })
          : await this._clickNewAgent(c, false);
      } catch (error) {
        if (isAgentsWorkspaceBindError(error) && !error.workspaceBindingFailure) {
  ...
  async _monitorParallelAgent(job, generation) {
    ...
    const recovered = await this._withJobLock(job, async () => {
      ...
      // One terminal probe at expiry; only explicit reap grants another monitoring budget.
    ...
    try {
      ...
      if (previousSelectedId && previousSelectedId !== job.agentId) {
        try {
          ...
          job.uiDiagnostic = { ...job.uiDiagnostic, selectionRestore: { state: 'failed', agentId: previousSelectedId, detail: String(error.message).slice(0, 500) } };
        }
      }
    }
  }
  ...
  async _collectParallelAgent(job) {
    ...
    try {
      ...
      try { await this._closeHistory(c); } finally { c.close(); }
    }
  }
  ...
  _reapWithoutResult(job, error, evidence) {
    ...
    job.reservationScope = job.effectiveExecution === 'fifo' ? 'global' : uncertainSubmissionReservationScope(job, error);
  ...
  async _readStableParallelEntry(job) {
    ...
    const stable = first.id === second.id && second.id === job.agentId
  ...
  async _reapParallelJobLocked(job, options = {}) {
    ...
    if (entry.showSpinner) {
      ...
      // An explicit reap may grant a fresh monitoring budget; automatic recovery never does.
  ...
  async _taskControlLocked(job, action, options) {
    if (action === 'reap') {
      ...
      if (result.state === 'not_parallel_reservation' && job.phase === 'orphaned') {
        ...
        ? 'The bound task has no recoverable reservation. Inspect its current state before cancelling.'
  ...
  async _waitComplete(c, timeoutMs = QUERY_TIMEOUT, baselineCount = 0, job = null, providerErrorBaseline = '') {
    ...
    while (Date.now() < deadline) {
      ...
      if (job) job.lastWaitObservation = { at: new Date().toISOString(), stop: s.stop, messageCount: s.messageCount, replyLength: s.replyLength, sawStop: sawStop || s.stop > 0 };
    ...
    if (job.phase === 'orphaned') {
      ...
      view.attention = 'Agent History proves that the underlying task ended, but its final reply has not been collected. Retry with reap, or explicitly abandon only if a missing reply is acceptable.';
        ...
        view.attention = job.agentId
          ...
          : 'FIFO is bound to an agentId. Use reap to collect or resume monitoring; use cancel with the exact expected_agent_id to stop running work.')
  ...
  _compactTaskView(job, { summary = false } = {}) {
    ...
    if (selection && Object.values(selection).every((value) => value === undefined)) view.modelSelection = null;
  ...
  _compactStatusCommon() {
    ...
    return {
      ...
      ...(workspace.workspaceBinding && workspace.workspaceBinding.ok === false
  ...
  _compactLifecycleView() {
    ...
    if (lifecycle.degradedReason || lifecycle.spawnErrorCode || lifecycle.error || lifecycle.supervisorError
      || lifecycle.needsAction || lifecycle.nextStep || lifecycle.retryable || lifecycle.status === 'failed') {
      return lifecycle;
    }
    return {
  ...
  async status(taskId = '', { detail = 'compact' } = {}) {
    ...
    const common = normalizedDetail === 'full' ? {
      ...
      recentTasks: [...this.tasks.values()]
        .filter((job) => !this.activeParallel.has(job.id))
        .slice(-10)
        .map((job) => this._compactTaskView(job, { summary: true })),
      unreadResultTaskIds: [...this.tasks.values()].filter((job) => isTerminalTask(job) && job.result != null && !job.resultCollectedAt).map((job) => job.id),
      taskRetentionLimit: 50,
  ...
  return [
    {
      ...
      description:
        ...
        'In Agents Window, initialization registers a missing local workspace through Cursor\'s project service and verifies its exact path before reporting ready. ' +
    ...
    bridgeInstance.environmentDelegationMode !== 'off' ? {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          timeout_ms: { type: 'integer', minimum: 30000, maximum: 900000, default: 600000, description: 'One monitoring budget after submission, shared by FIFO and automatic Agent recovery. Default 10 minutes, maximum 15 minutes; task status reports requested and effective budgets. Expiry probes the bound Agent once for completion; it does not cancel or resend work. Explicit reap may start a new budget for a still-running bound Agent, including FIFO.' },
    ...
    },
    {
      ...
      description:
        'Recover, collect a reply, close, or forget one exact persistent cursor_do session. reconcile checks the exact Agent twice and never resends. collect_result reads the completed reconciled turn from that Agent without sending a prompt; it may temporarily select that Agent. close prevents future sends but does not stop or delete the Cursor Agent. ' +
...
async function ensureBridgeCursor(targetBridge, reason) {
  return targetBridge._withWorkspaceAdmission(async () => {
    ...
    const r = await ensureCursorRunning({
      ...
      ...(targetBridge.workspaceConfirmationRequired ? { projectPath: null }
...
function toolErrorResult(error) {
  ...
  if (error && (error.uiDiagnostic || error.modelSelection || error.pickerRead || error.cdp || error.workspaceRecovery)) {
    const payload = {
      ...
      ...(error.workspaceRecovery ? { workspaceRecovery: error.workspaceRecovery } : {}),
    };
    return {
      ...
      try {
        result = await Promise.race([
          args && args.session_id
            ...
            : bridge.status(args && args.task_id, { detail }),
