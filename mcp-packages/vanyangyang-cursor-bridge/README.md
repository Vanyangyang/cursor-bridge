...
```powershell
npm install --prefix "$env:LOCALAPPDATA\cursor-bridge\npm" vanyangyang-cursor-bridge@0.1.4
```

The MCP bundle and executable entry are `dist/cursor-bridge.mjs`. The skills are `skills/cce-routing` and `skills/cursor-delegate`. The current AI client registers those paths itself, then restarts. Wrapper 0.1.4 embeds Cursor Bridge 6.0.6.
