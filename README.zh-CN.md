...
```text
Codex / Claude Code
   |  （逻辑上一个插件；每个对话/执行上下文可能各自拉起一份 stdio MCP adapter）
   v
cursor-bridge adapter（stdio MCP）  --IPC-->  用户级 Cursor lifecycle supervisor（单例）
                                               |
                                               +-- 负责检查/拉起/恢复带 CDP :9223 的 Cursor
                                               v
                                            Cursor 渲染进程  <-- CDP Runtime.evaluate + Input -- 各 adapter
```

- 每个 MCP 上下文可启动独立 adapter，但所有 adapter 共用一个用户级 lifecycle supervisor。
- supervisor 负责拉起和恢复带 CDP `:9223` 的 Cursor；Windows 上会脱离 Codex Job。adapter 断开不会关闭 Cursor。
- GUI 输入串行；FIFO 使用当前对话，`parallel_agent` 使用按 `agentId` 标识的独立顶层 Agent。
...
adapter 会请求 supervisor 确保 Cursor 可用。`CURSOR_BRIDGE_NO_AUTOLAUNCH=1` 可关闭自动拉起；`CURSOR_BRIDGE_INLINE_ENSURE=1` 仅用于排障，它会绕过 supervisor，在 Windows Job 场景不安全。Linux 无法自动探测时请设置 `CURSOR_EXE`。
...
- `cursor_do` 默认使用 `background=true`、`new_chat=true`；保存 `task_id`，再用 `cursor_status(task_id)` 收回。
...
| `CURSOR_BRIDGE_INLINE_ENSURE` / `CURSOR_BRIDGE_NO_SUPERVISOR` | 未设置 | 设为 `1` 时绕过单例 supervisor，在 adapter 进程内 ensure（仅排障；Windows Job 生命周期下不安全）。 |
| `CURSOR_BRIDGE_LIFECYCLE_DIR` | 平台用户状态目录 | 覆盖 supervisor 状态目录（pid/lock/boot-env）。Windows 默认 `%LOCALAPPDATA%\\cursor-bridge\\lifecycle`。 |
| `CURSOR_BRIDGE_SUPERVISOR_SOCK` | 派生 | 覆盖 supervisor IPC 端点（Windows 为 Named Pipe，其他平台为 Unix socket）。 |
...
`npm run build` 会产出 adapter 与 lifecycle-supervisor 两个 bundle；插件安装必须同时携带。
...
dist/                 # adapter 与 supervisor bundle
server.mjs            # MCP 工具与调度
launch-cursor.mjs     # Cursor/CDP 启动器
skills/               # 委托与策略规则
test/                 # 回归测试
```

插件运行 `dist/cursor-bridge.mjs`。发布流程见 [RELEASING.md](./RELEASING.md)。
