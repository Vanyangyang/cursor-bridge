...
安装后的插件还包含一份由 Codex 与 Claude Code 共用的 `cce-routing` Skill：陌生项目、未知实现位置、跨模块关系追踪会优先匹配 CCE；已知文件读取、测试、日志、构建、Git 与外部文档则继续使用更便宜的原生路径。

Claude Code 还会获得一个很窄的路由门禁，只处理高置信的所有权、调用链、数据流、注册关系以及“加载 → 运行时 → 保存”问题。prompt hook 会要求 Claude 先尝试 CCE；如果它仍选择冲突的 context-mode 收集工具，门禁最多拒绝两次并引导其改用 `cursor_context_engine`。一旦 CCE 被尝试，门禁立即解除；达到有界重定向次数后也会失败开放。它不会拦截 `Read`、`Grep`、`Glob`、`Bash`、`Agent`、编辑、测试、日志、构建、Git、外部文档或用户明确拒绝 Cursor 的请求。这样能显著提高自动选择命中率，但不会把模型控制的工具调用宣传成数学意义上的百分之百保证。
