...
- `cursor_status` lists CDP page titles only. It does not inspect page DOM. CCE reloads a DOM-blank Agents page once. On Windows normal runtime, reusing an Agents Window also requests a throttled, non-activating native presentation recovery. Already-visible windows are left untouched so active menus and popovers stay open; only a window that was actually hidden receives the compositor resize pulse needed to recover a white Electron surface.
...
[![Cursor Bridge Star History](https://raw.githubusercontent.com/Vanyangyang/cursor-bridge/master/assets/star-history.svg?v=0dec6e028ea1)](https://github.com/Vanyangyang/cursor-bridge)
