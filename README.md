...
>
> **Live-tested environment:** Windows 11 + Cursor **3.21.16**, verified from Cursor's embedded package metadata and the current-user uninstall registry. The 6.0.4 source and rebuilt bundle passed exact workspace binding, CCE and read-only FIFO `cursor_do` with Cursor Grok 4.6/high in both IDE and Agents Window. Agents Window also passed two simultaneous independent Agents, persistent-session restart recovery, unread-result collection, and `minimal` CCE followed by normal restoration. Both close orders restored exactly one last-closed window type. Native 6.0.4 host pickup remains pending. Requires Node.js 18+, Cursor installed and signed in, and a local project Cursor can open. macOS has not yet been live-tested.
