...
test('repository marketplace keeps Cursor Bridge stable and publishes Grok as an isolated plugin', () => {
  ...
  assert.equal(rootPackage.version, '5.10.0');
  ...
  assert.match(codexCursorManifest.version, /^5\.10\.0(?:\+codex\.)?/);
  assert.equal(claudeCursorManifest.version, '5.10.0');
  assert.match(serverSource, /const PLUGIN_VERSION = '5\.10\.0';/);
  ...
  assert.equal(claudeCursor?.version, '5.10.0');
...
test('compatibility history keeps 5.10.0 current for Cursor 3.19.13 and archives 5.9.1', () => {
  ...
  assert.equal(data.current.cursorBridgeVersion, '5.10.0');
  ...
  assert.equal(data.current.acceptance.readOnlyMcpSupervisor, '5.9.1-local-patch-native-status-init-fifo-verified; 5.10.0-native-pickup-pending');
  ...
  assert.match(data.current.acceptance.currentReleaseEvidence, /fresh-host-5.10.0-pickup-pending/);
  ...
  assert.match(english, /Current maintained baseline[\s\S]*3\.19\.13[\s\S]*5\.10\.0[\s\S]*fresh-host 5\.10\.0 pickup remains pending/);
  assert.match(chinese, /当前维护基线[\s\S]*3\.19\.13[\s\S]*5\.10\.0[\s\S]*新宿主加载仍待验收/);
  assert.match(englishReadme, /3\.19\.13[\s\S]*5\.10\.0[\s\S]*main[\s\S]*current/);
  assert.match(chineseReadme, /3\.19\.13[\s\S]*5\.10\.0[\s\S]*main[\s\S]*当前版本/);
