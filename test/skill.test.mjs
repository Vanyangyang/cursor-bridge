...
test('repository marketplace keeps Cursor Bridge stable and publishes Grok as an isolated plugin', () => {
  ...
  assert.equal(rootPackage.version, '5.9.1');
  ...
  assert.match(codexCursorManifest.version, /^5\.9\.1(?:\+codex\.)?/);
  assert.equal(claudeCursorManifest.version, '5.9.1');
  assert.match(serverSource, /const PLUGIN_VERSION = '5\.9\.1';/);
...
test('compatibility history keeps 5.9.1 current for Cursor 3.19.7 and archives 5.9.0', () => {
  ...
  assert.equal(data.current.cursorBridgeVersion, '5.9.1');
  ...
  assert.equal(data.current.acceptance.modelPreferences, 'live-tested-bundled-picker-flow-claude-fable-5.1-high; native-cce-pending');
  assert.equal(data.current.acceptance.readOnlyMcpSupervisor, '5.9.0-fresh-host-status-verified; 5.9.1-native-pickup-pending');
  ...
  assert.match(data.current.acceptance.currentReleaseEvidence, /bundled-picker-flow-live-verified/);
  assert.match(data.current.acceptance.currentReleaseEvidence, /fresh-host-cce-pickup-pending/);
  ...
  assert.match(english, /Current maintained baseline[\s\S]*3\.19\.7[\s\S]*5\.9\.1[\s\S]*native CCE pickup remains pending/);
  assert.match(chinese, /当前维护基线[\s\S]*3\.19\.7[\s\S]*5\.9\.1[\s\S]*原生 CCE 加载仍待验收/);
  assert.match(englishReadme, /3\.19\.7[\s\S]*5\.9\.1[\s\S]*main[\s\S]*current/);
  assert.match(chineseReadme, /3\.19\.7[\s\S]*5\.9\.1[\s\S]*main[\s\S]*当前版本/);
