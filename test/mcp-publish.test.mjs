...
if (command === "run" && args[1] === "build:mcp-packages") {
  ...
  writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "vanyangyang-cursor-bridge", version: "0.1.0" }));
  process.exit(0);
}
if (command === "pack") {
...
function runScenario(t, { lookup, mismatch = false, tag = "cursor-bridge-mcp--v0.1.0", token } = {}) {
...
test("Cursor generic MCP publisher rejects wrong tags, uncertain registry state, and real tokens", (t) => {
  for (const scenario of [
    ...
    { tag: "cursor-bridge-mcp--v0.1.0", lookup: "error" },
    { tag: "cursor-bridge-mcp--v0.1.0", lookup: "missing", token: "real-secret" },
  ]) {
