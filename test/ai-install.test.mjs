...
test('bilingual AI install playbooks share the same completion standard', () => {
  ...
  for (const content of [english, chinese]) {
    ...
    assert.match(content, /vanyangyang-cursor-bridge@0\.1\.4/);
    assert.match(content, /vanyangyang-grok-build-supervisor@0\.1\.0/);
    assert.match(content, /source: npm:vanyangyang-cursor-bridge@0\.1\.4 \| git-checkout \| repo-workspace/);
    ...
    assert.match(content, /this AI client decides how to register|当前 AI 客户端自己决定/);
    assert.match(content, /Restart this AI client|重启当前 AI 客户端/);
